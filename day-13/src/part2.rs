...
fn is_perfect_reflection(input: &Vec<Vec<&str>>, start: usize, len: usize) -> bool {
    ...
    let mut found_the_smudge = false;
    ...
    while first_pointer >= 0 && end_pointer < len {
        if input[first_pointer as usize] != input[end_pointer] {
            if found_the_smudge {
                ...
                let mut number_of_miss_matches = 0;
                for i in 0..input[end_pointer].len() {
                    if input[first_pointer as usize][i] != input[end_pointer][i] {
                        number_of_miss_matches += 1;
                        if number_of_miss_matches > 1 {
                            return false;
                        }
                    }
                }
