...
pub fn permutations(
    record: &mut String,
    groups: &Vec<i64>,
    memoization: &mut HashMap<(Vec<i64>, String), i64>,
) -> i64 {
    ...
    if let Some(unknown_idx) = record.find("?") {
        if remaining
            > (record.len() - unknown_idx - (groups.len() - partial_groups.len() - 1).max(0)) as i64
        {
            return 0;
        }
        if let Some(result) = match unknown_idx {
            0 => None,
            _ => memoization.get(&(
                partial_groups.clone(),
                record.clone().split_off(unknown_idx - 1),
            )),
        } {
            *result
        } else {
            ...
            if unknown_idx > 0 {
                memoization.insert(
                    (partial_groups, record.clone().split_off(unknown_idx - 1)),
                    result,
                );
            }
            result
...
pub fn process(input: &str) -> i64 {
    ...
    input
        .iter()
        .map(|record| permutations(&mut record.clone().0, &record.1, &mut HashMap::new()))
        .sum::<i64>()
}
